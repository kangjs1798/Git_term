...
class CuteInterpreter(object):
    ...
    def run_arith(self, arith_node):
        ...
        def calc_arith(node1,operator, node2):
            if operator.type is TokenType.PLUS:
                ari = int(node1.value) + int(node2.value)
            elif operator.type is TokenType.MINUS:
                ari = int(node1.value) - int(node2.value)
            elif operator.type is TokenType.TIMES:
                ari = int(node1.value) * int(node2.value)
            elif operator.type is TokenType.DIV:
                if node2.value is "0":
                    print("Can't divide by zero")
                    return None
                ari = float(node1.value) / float(node2.value)

            result = Node(TokenType.INT, ari)
            if node2.next is not None:
                return calc_arith(result, operator, node2.next)
            else:
                return result

        def calc_compare(node1, operator, node2):
            oper1 = node1
            oper2 = node2
            if operator.type is TokenType.GT:
                while oper2 is not None:
                    if eval(oper1.value + "<=" + oper2.value):
                        return False
                    oper1 = oper2
                    oper2 = oper2.next
            elif operator.type is TokenType.LT:
                while oper2 is not None:
                    if eval(oper1.value + ">=" +  oper2.value):
                        return False
                    oper1 = oper2
        ...
        if temp1.type is not TokenType.INT or temp2.type is not TokenType.INT:
            print("Calc Error!! operand is not INT")
            return None
        else:
            if arith_node.type in [TokenType.PLUS, TokenType.MINUS, TokenType.TIMES, TokenType.DIV]:
                return calc_arith(temp1, arith_node, temp2)

            elif arith_node.type in [TokenType.GT, TokenType.LT, TokenType.EQ]:
                if calc_compare(temp1, arith_node, temp2):
    ...
    def run_func(self, func_node):
        ...
        """
        "Condition을 체크하는 함수"
        "조건 노드가 참이고, 리턴 노드가 존재하면, 리턴노드의 마지막 노드를 리턴"
        "리턴노드가 존재하지 않는다면 트루 노드 리턴"
        "false일 경우 recursion을 이용해 다음 조건 노드 검사"
        """
        def check_condition(chk_node):
            temp = chk_node.value
            chk_expr = self.run_expr(temp)
            if chk_expr.type is TokenType.TRUE:
                if temp.next is None:
                    return self.TRUE_NODE
                return find_last(temp)
            else:
                if chk_node.next is None:
                    return None
                return check_condition(chk_node.next)


        "노드에 연결된 마지막 노드를 찾아주는 재귀 함수"
        def find_last(node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            temp = self.run_expr(rhs1)
            if not is_quote_list(temp):
                print ("cdr error!")
            if pop_node_from_quote_list(temp).next is not None:
                result = pop_node_from_quote_list(temp).next
            else:
                result = None;
            ...
            if expr_rhs2.type is not TokenType.LIST:
                print("cons error!")

            if not list_is_null(expr_rhs2):
                temp1 = pop_node_from_quote_list(expr_rhs2)
            else:
                temp1 = None


            if expr_rhs1.type is TokenType.LIST:
                temp2 = expr_rhs1.value.next
                temp2.set_last_next(temp1)
            else:
                temp2 = expr_rhs1
                temp2.next = temp1
            return create_quote_node(temp2, True)
    ...
    def run_list(self, l_node):
        ...
        if op_code.type in [TokenType.PLUS, TokenType.MINUS, TokenType.TIMES,TokenType.DIV,TokenType.LT, TokenType.GT, TokenType.EQ]:
            return self.run_arith(op_code)
        if op_code.type is TokenType.QUOTE:
            return l_node
        if op_code.type is TokenType.DEFINE:
            return self.run_define(op_code)
        else:
